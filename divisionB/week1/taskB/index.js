 ...
 * линии. Требуется выяснить, мимо какого наименьшего количества промежуточных станций необходимо проехать Вите
 * по кольцу, чтобы добраться с работы домой.
 *
 * Формат ввода
...
async function circleUndergroundPath(N, i, j) {
  const min = Math.min(i, j)
  const max = min === i ? j : i
  return Math.min(max - min - 1, min - 1 + N - max)
}

;(async () => {
  console.log(await circleUndergroundPath(...(await readInput('input.txt'))))
})()
