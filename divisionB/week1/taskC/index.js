 ...
 * потом год) и американски (сначала месяц, потом день, потом год). Системный администратор поменял дату на одном
 * из бэкапов и сейчас хочет вернуть дату обратно. Но он не проверил, в каком формате дата используется в системе.
...
async function isSingleFormatDate(x, y, z) {
  return x > 12 || y > 12 || x === y ? 1 : 0
}

;(async () => {
  console.log(await isSingleFormatDate(...(await readInput('input.txt'))))
})()
