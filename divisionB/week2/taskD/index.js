...
async function getImportantBlocks(L, K, ...positions) {
  const center = (L - 1) / 2
  let leftEdgeHolder = positions[0]
  let rightEdgeHolder = positions[K - 1]

  for (let i = 1; i < positions.length; i++) {
    if (positions[i] === center) {
      return center
    } else if (positions[i] < center) {
      leftEdgeHolder = positions[i]
    } else if (positions[i] > center && positions[i] < rightEdgeHolder) {
      rightEdgeHolder = positions[i]
    }
  }
  return `${leftEdgeHolder} ${rightEdgeHolder}`
}

;(async () => {
  console.log(await getImportantBlocks(...(await readInput('input.txt'))))
})()
